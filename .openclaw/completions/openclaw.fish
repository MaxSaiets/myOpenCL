complete -c openclaw -n "__fish_use_subcommand" -a "mcp" -d 'Manage embedded Pi MCP servers'
...
complete -c openclaw -n "__fish_use_subcommand" -a "config" -d 'Non-interactive config helpers (get/set/unset/file/validate). Run without subcommand for guided setup.'
...
complete -c openclaw -n "__fish_seen_subcommand_from onboard" -l kimi-code-api-key -d 'Kimi Code API key'
...
complete -c openclaw -n "__fish_seen_subcommand_from plugins" -a "install" -d 'Install a plugin (path, archive, npm spec, or marketplace entry)'
complete -c openclaw -n "__fish_seen_subcommand_from plugins" -a "update" -d 'Update installed plugins (npm and marketplace installs)'
